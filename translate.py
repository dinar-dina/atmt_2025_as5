...
from seq2seq.decode import beam_search_decode, decode, beam_search_relative_pruning, beam_search_node_pruning
...
def get_args():
    ...
    parser.add_argument('--decode-mode',default='baseline',choices=['baseline', 'relative', 'node'],help='Decoding strategy: baseline beam search, relative pruning, or node pruning.')
...
def main(args):
    ...
    args = args_loaded
    # Automatically rename log file based on beam size
    ...
    for batch in tqdm(batch_iter(src_encoded, args.batch_size)):
        ...
        with torch.no_grad():
            ...
            if args.beam_size == 1:
                ...
                elif args.decode_mode == 'relative':
                    prediction = beam_search_relative_pruning(
                    model=model,
                    src_tokens=src_tokens,
                    src_pad_mask=src_pad_mask,
                    max_out_len=args.max_len,
                ...
                elif args.decode_mode == 'node':
                    prediction = beam_search_node_pruning(
                        model=model,
                        src_tokens=src_tokens,
                        src_pad_mask=src_pad_mask,
                        max_out_len=args.max_len,
    ...
    #----------------------------------------
        ...
        print(f"[Beam={args.beam_size}] Decoding time for 1 sentence: {end_dec - start_dec:.4f}s")
    #------------------------------------------
